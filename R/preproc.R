...
#' @param calib This is the signal you wish to use as the calibrant. This signal must be compatible with [metabom8::calibrate()]. For urine spectra, *tsp* is the main calibrant.
#' @param linw This argument provides import for [metabom8::lw()], a function that calculates the line width of peaks. Spectra with peaks that have small line widths have sharper and more precise results which is more desirable. A maximum cutoff of 1 ensures spectra contain robust results. Consider omitting spectra with line widths over 1.
#' @param bline Default is set to `TRUE`. This argument calls on [metabom8::bcor()], a baseline correcting function to smooth the spectral baselines and remove the influence of broad peaks.
...
preproc <- function(X, ppm, meta, bline = T, flip = F, cali = F, calib = 'tsp', linw = 1.0, lowCut = 0.25, watCut = c(4.5,5), ureCut = c(5.6,6), uppCut = 9.5, noi_sh = c(9.5, 11)){
  ...
  if (is.null(dim(X))){
    if (is.null(length(X))){
      ...
      if (cali){
        ...
        Xc <- metabom8::calibrate(Xf, ppm, type = calib)
        cat('\033[1;32mDone.\n\033[0m')
      } else {
      ...
      lwd <- metabom8::lw(Xc,ppm,shift = c(-0.1,0.1), sf = meta$a_SFO1)
      lwB <- lwd<linw
      if (all(lwB)){
      ...
      idx_rm <- c(metabom8::get_idx(range = c(min(ppm), lowCut), ppm), metabom8::get_idx(range = watCut, ppm), metabom8::get_idx(range = ureCut, ppm))
      ...
      X <- X[-metabom8::get_idx(range = c(uppCut, max(ppm)), ppm)]
      ppm <- ppm[-metabom8::get_idx(range = c(uppCut, max(ppm)), ppm)]
