...
hmNorm <- function(X, noi, int_binwid = 0.1, alpha = c(0.1, 2), use_median = NULL, tol = 1e-5){
    if (is.null(dim(X))){
      ...
      if (is.null(use_median)){
        stop("Please define which reference to use. To use median: use_median = TRUE; to use first spectra: use_median = FALSE.")
      ...
      Xs <- t(vapply(seq_len(nrow(X)), function(j){
        Xc <- X[j,]
        n <- noi[j]
        Xc[(Xc <= n)] = NA
        return(Xc)
      ...
      if (use_median){
        ...
        dilfs <- vapply(seq_len(nrow(Xs)), function(i){
          a <- alpha[1]
          b <- alpha[2]
          gr <- (sqrt(5)+1)/2
          c <- b-(b-a)/gr
