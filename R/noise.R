...
noise <- function(X_OG, ppm_OG, sh = c(9.5,11), sd_mult = 5, method = 'simple'){
  ...
  if (method!='simple' & method!= 'topspin'){
    ...
    if (is.null(nrow(X_OG))){
      rm <-  sd_mult*stats::sd(X_OG[metabom8::get_idx(sh, ppm_OG)])+(mean((X_OG[metabom8::get_idx(sh, ppm_OG)]), trim = 0.05))
    } else {
      rm <- apply(X_OG, 1, function(i){
        rm <- sd_mult*stats::sd(i[metabom8::get_idx(sh, ppm_OG)])+(mean((i[metabom8::get_idx(sh, ppm_OG)]), trim = 0.05))
        return(rm)
      })
    }
  } else if (method == "topspin"){
    if (is.null(nrow(X_OG))){
      x <- X_OG[metabom8::get_idx(sh, ppm_OG)]
      n <- floor(length(x)/2)
      N <- n*2
      le <- length(x)
      ...
      rm <- t(sapply(1:nrow(X_OG), function(i){
        x <- X_OG[i, metabom8::get_idx(sh, ppm_OG)]
        n <- floor(length(x)/2)
        N <- n*2
        le <- length(x)
        out <- sapply(1:n, function(i){
